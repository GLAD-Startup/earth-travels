    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
      {destinations.map((dest) => {
        const matchingPackages = PACKAGES.filter(
          (pkg) => pkg.destinationId.toLowerCase() === dest.id.toLowerCase()
        );
        const dynamicPackageCount = matchingPackages.length;
        const dynamicStartingPrice = dynamicPackageCount > 0
          ? Math.min(...matchingPackages.map((p) => p.pricePerPerson))
          : dest.startingPrice;
              ...
              <div className="pt-4 border-t border-charcoal/5 flex flex-col gap-3 mt-auto">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-sans text-charcoal/45 flex items-center gap-1.5">
                    ...
                    {dynamicPackageCount} {dynamicPackageCount === 1 ? "package" : "packages"}
                  </span>
                  <span className="font-sans text-accent font-bold">
                    From ₹{dynamicStartingPrice.toLocaleString("en-IN")}
                  </span>
                </div>
                <Link
