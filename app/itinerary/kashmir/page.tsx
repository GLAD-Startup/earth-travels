    ...
    <div className="bg-[#080C14] min-h-screen text-white select-none">
      ...
      >
        <div className="max-w-7xl mx-auto w-full relative z-10 flex flex-col gap-6">
          ...
          <div>
            <h1 className="hero-slide-up font-display text-4xl md:text-[80px] font-normal italic text-white leading-none mb-2">{itinerary.tagline}</h1>
            <h2 className="hero-slide-up font-display text-2xl md:text-[48px] font-bold text-white/90 leading-none">{itinerary.title}</h2>
          </div>
          <div className="hero-slide-up glass p-5 rounded-2xl border border-white/10 max-w-4xl mt-4 bg-[#080C14]/50 backdrop-blur-md">
            <div className="grid grid-cols-2 md:grid-cols-5 gap-6 text-center md:divide-x divide-white/10">
              {[
                ...
                <div key={item.label} className="flex flex-col gap-1 pt-2 md:pt-0">
                  <span className="text-white/40 font-sans text-[10px] uppercase">{item.label}</span>
      ...
      {/* Mobile Day Bar */}
      <div className="sticky top-[72px] z-30 w-full bg-[#080C14]/90 backdrop-blur-[24px] border-b border-white/5 py-3 px-6 text-center lg:hidden">
      ...
      <section className="max-w-7xl mx-auto px-6 py-20 relative z-10">
        <div className="flex flex-col lg:flex-row gap-12 items-start">
          ...
          <aside className="hidden lg:block w-40 shrink-0 sticky top-28 self-start">
            <h4 className="font-mono text-[10px] text-white/40 uppercase tracking-[0.2em] mb-5 font-bold">Progress Map</h4>
              ...
              {itinerary.days.map((day) => {
                ...
                return (
                  <button key={day.dayNumber} onClick={() => handleScrollToDay(day.dayNumber)} className="flex items-center gap-4 text-left group transition-all">
                    <span className={`w-5 h-5 rounded-full flex items-center justify-center font-mono text-[9px] font-bold border transition-all z-10 ${isActive ? "bg-[#D4A017] text-[#080C14] border-transparent scale-110 shadow-lg" : "bg-midnight border-white/20 text-white/50 group-hover:border-white/50"}`}>
                      {day.dayNumber}
                    </span>
                    <span className={`text-xs font-sans transition-colors duration-300 ${isActive ? "text-[#D4A017] font-bold" : "text-white/50 group-hover:text-white"}`}>
                      Day {day.dayNumber}
                    </span>
                  </button>
                );
          ...
          <div className="flex-grow w-full lg:max-w-[700px] flex flex-col gap-2">
            {itinerary.days.map((day) => (
              <div key={day.dayNumber} id={`day-section-${day.dayNumber}`} data-day={day.dayNumber} className="day-scroll-section relative pt-20 pb-10 border-b border-white/6 overflow-hidden flex flex-col gap-6">
                <div className="absolute top-8 left-[-10px] pointer-events-none select-none opacity-[0.04] z-0">
                  <span className="font-mono text-[120px] font-black leading-none text-white">DAY {String(day.dayNumber).padStart(2, "0")}</span>
                </div>
                <div className="relative z-10 pl-4 border-l-2 border-[#D4A017] mt-4 flex flex-col items-start gap-1">
                  ...
                  <h3 className="font-display text-3xl md:text-4xl font-normal text-white mt-1 leading-tight">Day {day.dayNumber} — {day.title}</h3>
                </div>
                <p className="font-sans text-sm text-white/70 italic max-w-xl pl-4 relative z-10">{day.tagline}</p>
                <div className="relative z-10 pl-4 flex flex-col gap-4 mt-2">
                  {day.activities.map((act, index) => (
                    <GlassCard key={index} hover={false} className="p-4 bg-white/5 border border-white/5 hover:border-white/10 rounded-xl flex items-start gap-4 transition-colors">
                      ...
                      <div className="flex-grow">
                        <h4 className="font-sans text-[15px] font-semibold text-white leading-tight mb-1">{act.title}</h4>
                        <p className="font-sans text-xs text-white/60 leading-relaxed">{act.description}</p>
                      </div>
                    </GlassCard>
                  ))}
                </div>
                <div className="relative z-10 pl-4 mt-2">
                  <div className="p-4 border-l border-[#D4A017] bg-[#080C14]/60 backdrop-blur-md rounded-r-xl border-y border-r border-white/5">
            ...
            <div className="pt-16">
              ...
              <GlassCard hover={false} className="p-6 md:p-8 bg-[#080C14]/40 border border-white/10 rounded-[20px]">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div>
                    ...
                    <ul className="flex flex-col gap-3 text-xs md:text-sm text-white/80">
                  ...
                  <div className="border-t md:border-t-0 md:border-l border-white/5 pt-8 md:pt-0 md:pl-8">
                    ...
                    <ul className="flex flex-col gap-3 text-xs md:text-sm text-white/60">
            ...
            <div className="pt-16">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {itinerary.hotels.map((hotel, idx) => (
                  <GlassCard key={idx} hover={true} className="p-0 overflow-hidden bg-midnight/35 border border-white/10 hover:border-[#D4A017]/30 transition-all duration-300 rounded-2xl flex flex-col justify-between group">
                    ...
                    <div className="p-4">
                      <div className="flex items-center justify-between text-[#D4A017] text-xs mb-1 font-semibold">
                        ...
                        <span className="text-white/40 text-[9px] font-mono tracking-wider uppercase">{hotel.type}</span>
                      </div>
                      <h4 className="font-sans text-sm font-bold text-white leading-tight mb-1">{hotel.name}</h4>
                      <p className="font-sans text-[11px] text-white/50">📍 {hotel.location}</p>
                    </div>
                  </GlassCard>
                ))}
              </div>
          ...
          <aside className="w-full lg:w-80 shrink-0 sticky top-28 self-start mt-12 lg:mt-0">
            <GlassCard hover={false} className="p-6 md:p-8 bg-[#080C14]/80 border border-white/10 shadow-2xl rounded-[20px]">
              <h3 className="font-display text-xl font-bold text-white mb-6 border-b border-white/5 pb-4">Book This Package</h3>
              <form onSubmit={(e) => e.preventDefault()} className="flex flex-col gap-5">
                <div className="flex flex-col gap-2">
                  <label className="font-mono text-[10px] text-white/50 uppercase tracking-widest pl-1">Departure Date</label>
                  <input type="date" value={departureDate} onChange={(e) => handleDepartureChange(e.target.value)} min={new Date().toISOString().split("T")[0]} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-[#D4A017] font-sans" required />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="font-mono text-[10px] text-white/50 uppercase tracking-widest pl-1">Return Date ({itinerary.duration})</label>
                  <input type="date" value={returnDate} onChange={(e) => setReturnDate(e.target.value)} min={departureDate || new Date().toISOString().split("T")[0]} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-[#D4A017] font-sans" required />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  {[{ label: "Adults", val: adults, set: setAdults, min: 1 }, { label: "Children", val: children, set: setChildren, min: 0 }].map(({ label, val, set, min }) => (
                    <div key={label} className="flex flex-col gap-2">
                      <label className="font-mono text-[10px] text-white/50 uppercase tracking-widest pl-1">{label}</label>
                      <div className="flex items-center justify-between bg-white/5 border border-white/10 rounded-xl px-2 py-1 h-[40px]">
                        <button type="button" onClick={() => set((p: number) => Math.max(min, p - 1))} className="w-6 h-6 rounded flex items-center justify-center hover:bg-white/10 text-white font-mono font-bold">-</button>
                        <span className="font-mono text-xs font-semibold text-white/80">{val}</span>
                        <button type="button" onClick={() => set((p: number) => p + 1)} className="w-6 h-6 rounded flex items-center justify-center hover:bg-white/10 text-white font-mono font-bold">+</button>
                      </div>
                    </div>
                  ))}
                </div>
                <div className="flex flex-col gap-2">
                  <label className="font-mono text-[10px] text-white/50 uppercase tracking-widest pl-1">Sharing Class</label>
                  <div className="flex flex-col gap-2.5 mt-1 pl-1">
                    {[{ value: "twin", label: "Twin Sharing stay" }, { value: "double", label: "Double Room stay" }, { value: "single", label: "Single Supplement (+₹8,000)" }].map((opt) => (
                      <label key={opt.value} className="flex items-center gap-2.5 text-xs text-white/70 hover:text-white cursor-pointer">
                        <input type="radio" name="room" checked={roomType === opt.value} onChange={() => setRoomType(opt.value)} className="sr-only" />
                        <span className={`w-3.5 h-3.5 rounded-full border flex items-center justify-center transition-colors ${roomType === opt.value ? "border-[#D4A017] bg-[#D4A017]" : "border-white/20"}`}>
                          {roomType === opt.value && <span className="w-1.5 h-1.5 rounded-full bg-midnight" />}
                        </span>
                        <span>{opt.label}</span>
                      </label>
                    ))}
                ...
                <div className="pt-4 border-t border-white/5 flex flex-col gap-2.5 text-xs text-white/50 font-sans">
                ...
                <a href={whatsAppLink} target="_blank" rel="noopener noreferrer" className="text-center font-sans text-xs text-white/50 hover:text-white transition-colors block underline underline-offset-4 mt-1">
                  or WhatsApp us →
                </a>
              </form>
            </GlassCard>
        ...
        <div className="pt-24 border-t border-white/5 mt-20">
          <h3 className="font-display text-3xl font-normal text-white mb-10 pl-4 border-l-2 border-[#D4A017]">You Might Also Love</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {relatedPackages.map((pkg, idx) => (
              <GlassCard key={idx} hover={true} className="p-0 overflow-hidden bg-midnight/35 border border-white/10 group rounded-2xl flex flex-col justify-between">
                <div className="relative h-44 w-full overflow-hidden">
                  ...
                  <span className="absolute bottom-3 right-3 z-10 bg-midnight/80 border border-white/10 text-white font-mono text-[9px] px-2.5 py-1 rounded-full uppercase tracking-wider">⏱ {pkg.duration.nights}N/{pkg.duration.days}D</span>
                </div>
                <div className="p-5 flex-grow flex flex-col justify-between">
                  <div>
                    ...
                    <h4 className="font-sans text-base font-bold text-white mb-4 line-clamp-1 group-hover:text-[#D4A017] transition-colors leading-tight">{pkg.name}</h4>
                  </div>
                  <div className="pt-3 border-t border-white/5 flex items-center justify-between mt-auto">
                    ...
                    <Link href={pkg.itineraryPage} className="text-white text-xs font-mono group-hover:text-[#D4A017]">Explore →</Link>
                  </div>
                </div>
              </GlassCard>
            ))}
